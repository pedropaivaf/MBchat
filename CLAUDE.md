...
- **Copiar rapido no hover (v1.4.53)** — Cada mensagem em `_append_message` ganha tag `msg_N` cobrindo `body_start`->`body_end` + marks `mstart_N`/`mend_N` (GroupChat: `gmstart_N`/`gmend_N`). `_msg_ranges_idx` guarda tuplas `(start_mark, end_mark)` e `_msg_data` guarda `{msg_id, sender, text, is_mine, timestamp}`. Um unico `_hover_copy_btn` (`tk.Label` com icone MDL2 `\uE8C8` cinza minimalista) e reusado por janela, placed via `place(in_=chat_text)` ao lado do horario. `tag_bind('<Enter>')` mostra; `<Leave>` agenda `_schedule_hover_hide(180ms)` que e cancelado pelo `<Enter>` no botao. Clique copia `_msg_data[idx]['text']` e da feedback visual `\u2714` por 600ms. STATUS: posicionamento ainda com bug visual em algumas situacoes — botao aparece grudado na borda direita em vez de ao lado do horario (dlineinfo da linha do header retorna width que nao reflete fim real do texto; pendente corrigir usando mark dedicado `time_end_N` capturado antes do `\n`). Feature funcional mas UX prejudicada.
