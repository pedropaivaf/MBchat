...
- **Copiar rapido no hover (v1.4.53, area ampliada em v1.4.54, delay ampliado em v1.4.55)** — Cada mensagem em `_append_message` ganha tag `msg_N`. A partir de v1.4.54 a tag cobre `header_start -> body_end` (nome + horario + corpo inteiro do balao), de forma que o hover dispara em qualquer ponto visual da mensagem; antes cobria so o corpo. Marks `mstart_N`/`mend_N` (ChatWindow) / `gmstart_N`/`gmend_N` (GroupChat) continuam apontando para `body_start`/`body_end` para preservar o modo selecao (long-press so sobre o corpo). `_msg_ranges_idx` guarda tuplas `(start_mark, end_mark)` e `_msg_data` guarda `{msg_id, sender, text, is_mine, timestamp}`. Um unico `_hover_copy_btn` (`tk.Label` com icone MDL2 `\uE8C8` cinza minimalista) e reusado por janela, placed via `place(in_=chat_text)`. `tag_bind('<Enter>')` mostra; `<Leave>` agenda `_schedule_hover_hide` que e cancelado pelo `<Enter>` no botao. Delay do hide ampliado para 600ms em v1.4.55 (antes 180ms) porque o mouse transitando da tag da mensagem para o label do botao estava sendo interrompido pelo hide — o botao sumia antes do click. Clique copia `_msg_data[idx]['text']` e da feedback visual `\u2714` por 600ms.
- **Notificacao abre apenas a janela do chat (v1.4.55)** — Antes, clicar no toast do Windows roteava para `_restore_and_open`, que sempre chamava `self.root.deiconify()` e trazia a main window junto. Agora o listener de instancia loopback roteia comandos `OPEN:{uid}` para o novo `_open_from_notification(peer)`, que abre/surface apenas a `ChatWindow`/`GroupChatWindow` alvo **sem** restaurar o root. Helper `_bring_chat_window_to_front(win)` faz `lift()` + `focus_force()` + `attributes('-topmost', True)` + `after(200, lambda: attributes('-topmost', False))`. Comandos `SHOW:` (tray click) continuam em `_restore_and_open` para preservar o comportamento de restaurar a main window pelo icone do tray.
- **Dialog About modular (v1.4.56)** — Menu renomeado de "Ajuda" para "Sobre" e o item do submenu passa a se chamar apenas "MB Chat". `_show_about` agora cria um `tk.Toplevel` custom (nao mais `messagebox.showinfo`) dividido em helpers pequenos: `_build_about_dialog`, `_build_about_header` (icone 72x72 + nome + versao + assinatura MB Contabilidade), `_build_about_body` (subtitulo + lista de features com bullets unicode), `_build_about_footer` (botoes Autor + OK). Novo botao "Autor" chama `_open_author_site` que abre `https://pedropaivaf.dev/` via `webbrowser.open`. Dialog usa `_center_window` + `_apply_rounded_corners` (fallback silencioso em Win10) + `_force_taskbar_entry`, todas APIs compat Win10+. Esc ou OK fecham. Padrao modular intencional (um metodo por secao) conforme diretriz do usuario: menos acoplamento por metodo grande, mais independencia por acao.
