...
- **Departamentos/Equipes** — Preferencias > Conta permite selecionar departamento. Opcoes: (Nenhum), Administrativo, Fiscal, Contabil, TI, Comercial, Pessoal, CS, Marketing, Recepcao. Departamento exibido como badge `[Setor]` em azul ao lado do nome no TreeView. Campo `department` no UDP announce (enviado E recebido) e na tabela `contacts`. Mudanca no combo aplica na hora via `_on_dept_changed` -> `discovery.update_department()` -> announce imediato.
