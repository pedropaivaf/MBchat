...
- **Departamentos/Equipes** — Preferencias > Conta permite selecionar departamento. Opcoes (ordem alfabetica, com "(Nenhum)" como sentinela): (Nenhum), Administrativo, CS, Comercial, Contabil, Fiscal, Marketing, Pessoal, Processos, Recepcao, TI. Departamento exibido como badge `[Setor]` em azul ao lado do nome no TreeView. Campo `department` no UDP announce (enviado E recebido) e na tabela `contacts`. Mudanca no combo aplica na hora via `_on_dept_changed` -> `discovery.update_department()` -> announce imediato.
